...
// Small UI bits
// -----------------------
const Bullet = () => <span className="w-2 h-2 bg-green-500 rounded-full mr-2 inline-block" />
...
const ServiceDetail = () => {
      ...
      <div className="min-h-screen bg-white">
        ...
        <main className="py-12">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <Link to="/" className="text-green-600 underline">Go back home</Link>
          </div>
        </main>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* SEO / JSON-LD */}
      <Helmet>
        <title>{`${service.title} in Halifax | JunkNerds`}</title>
        <meta name="description" content={service.description} />
        <link rel="canonical" href={`https://junknerds.ca/services/${slug}`} />
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          "itemListElement": [
            ...
            { "@type": "ListItem", "position": 2, "name": "Services", "item": "https://junknerds.ca/services" },
            { "@type": "ListItem", "position": 3, "name": service.title, "item": `https://junknerds.ca/services/${slug}` }
          ]
        })}</script>
        {service.faqs?.length ? (
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {service.features && (
              <ul className="mt-4 flex flex-wrap gap-2">
                {service.features.map((f: string) => (
                  <li key={f} className="text-sm bg-green-50 text-green-700 border border-green-200 px-3 py-1 rounded-full">{f}</li>
                ))}
              </ul>
            )}
          </header>
          ...
          <section className="grid lg:grid-cols-3 gap-8 mb-10">
            <div className="lg:col-span-2 space-y-10">
              <div>
                <h2 className="text-xl font-semibold text-gray-900 mb-3">Items we remove</h2>
                <ul className="grid sm:grid-cols-2 gap-2">
                  {service.items.map((item: string) => (
                    <li key={item} className="flex items-start gap-2 text-gray-700 leading-6">
                      {/* dot */}
                      <span
                        className="mt-1.5 h-2 w-2 rounded-full bg-emerald-500 shrink-0"
                        aria-hidden
                      />
                      <span>{item}</span>
                    </li>
                  ...
                  <ol className="space-y-3">
                    {service.process.map((step: string, i: number) => (
                      <li key={i} className="flex">
                        <span className="mr-3 mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-green-600 text-white text-sm font-semibold">{i + 1}</span>
            ...
            <aside className="lg:sticky lg:top-24 space-y-4">
              <Card className="rounded-2xl border shadow-sm p-6">
                ...
                <div className="mt-4 flex flex-col gap-2">
                  <Button asChild className="bg-green-600 hover:bg-green-700"><Link to="/appointment">Book Now</Link></Button>
                  <Button asChild variant="outline" className="border-green-600 text-green-600 hover:bg-green-50"><a href="tel:+19024128566">Call (902) 412-8566</a></Button>
                </div>
              </Card>
            </aside>
          </section>
